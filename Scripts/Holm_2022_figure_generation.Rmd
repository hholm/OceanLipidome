...
Joined_metadata <- Joined_metadata[-1,] # drop the first row (units info)
...
labels <- c("sqrt Fluorescence", "Depth", "Salinity","sqrt NO3+NO2","PAR",
            "Days since summer soltice","Temperature")
...
n_dgdg <- ggplot(subset(Joined_metadata, Mixed_Layer == "In Mixed Layer")) +
  ...
  theme_pubr() +
  ylab("%EPA containing species (DGDG)") +
  xlab("Water temperature (°C)") +
  theme()
...
n_mgdg <- ggplot(subset(Joined_metadata, Mixed_Layer == "In Mixed Layer")) +
  ...
  theme_pubr() +
  ylab("%EPA containing species (MGDG)") +
  xlab("Water temperature (°C)")
